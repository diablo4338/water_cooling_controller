...
TEST_CASE("fsm pairing flow transitions", "[fsm]") {
    reset_fsm();

    TEST_ASSERT_TRUE(fsm_dispatch(FSM_EVT_PAIR_START, BLE_HS_CONN_HANDLE_NONE));
    TEST_ASSERT_EQUAL(FSM_STATE_PAIRING, fsm_get_state());

    TEST_ASSERT_TRUE(fsm_dispatch(FSM_EVT_PAIR_FINISH, 12));
    TEST_ASSERT_EQUAL(FSM_STATE_PAIRED_UNAUTH, fsm_get_state());
    TEST_ASSERT_EQUAL(12, fsm_get_term_conn_handle());
    TEST_ASSERT_EQUAL(12, fsm_get_auth_conn_handle());
}

TEST_CASE("fsm pairing timeout resets", "[fsm]") {
    reset_fsm();

    TEST_ASSERT_TRUE(fsm_dispatch(FSM_EVT_PAIR_START, BLE_HS_CONN_HANDLE_NONE));
    TEST_ASSERT_EQUAL(FSM_STATE_PAIRING, fsm_get_state());

    TEST_ASSERT_TRUE(fsm_dispatch(FSM_EVT_PAIR_TIMEOUT, BLE_HS_CONN_HANDLE_NONE));
    TEST_ASSERT_EQUAL(FSM_STATE_UNPAIRED_IDLE, fsm_get_state());
